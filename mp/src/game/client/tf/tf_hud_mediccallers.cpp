...
void CTFMedicCallerPanel::PaintBackground( void )
{
	...
	Vector vecDelta = vecTarget - MainViewOrigin(0);
	...
	if( !bOnscreen || iX < halfWidth || iX > ScreenWidth()-halfWidth )
	{
		...
		if( MainViewRight(0).Dot( vCenter - MainViewOrigin(0) ) > 0 )
		{
	...
	else
	{
		...
		trace_t	tr;
		UTIL_TraceLine( vecTarget, MainViewOrigin(0), MASK_OPAQUE, NULL, COLLISION_GROUP_NONE, &tr );
		if ( tr.fraction >= 1.0f )
		{
			m_bOnscreen = true;
			SetAlpha( 0 );
